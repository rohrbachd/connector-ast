  ...
  async verifyPresentation(
    ...
    try {
      ...
      if (invalidCredentials.length > 0) {
        return {
          valid: false,
          error: 'Invalid credentials in presentation',
  ...
  async addTrustAnchor(anchor: TrustAnchor): Promise<void> {
    ...
    `,
      [
        ...
        anchor.type,
        anchor.did,
        anchor.certificatePem,
        anchor.publicKeyJwk,
        anchor.trustFramework,
        anchor.status,
        anchor.validFrom,
        anchor.validUntil,
      ],
...
class ODRLPolicyEngine implements PolicyDecisionPoint {
  ...
  async evaluate(context: PolicyEvaluationContext): Promise<PolicyDecision> {
    ...
    const prohibitionResults = await Promise.all(
      resolvedPolicy.prohibition?.map(prohibition =>
        this.evaluateProhibition(prohibition, context),
      ) || [],
    );
    ...
    if (permission.constraint) {
      const constraintResults = await Promise.all(
        permission.constraint.map(constraint =>
          this.constraintEvaluator.evaluate(constraint, context),
        ),
      );
